...
_MODEL_DISPLAY_NAMES = {
    ...
    "openai_codex/gpt-5.5": "GPT-5.5 Codex",
    "open_router/openai/gpt-5.5": "GPT-5.5 Codex",
}


def _human_display_name(ref: str) -> str:
    """Converte 'deepseek/deepseek-v4-pro (no thinking)' -> 'DeepSeek V4 Pro (no thinking)'."""
    no_thinking = False
    if ref.endswith(" (no thinking)"):
        ref = ref[:-len(" (no thinking)")]
        no_thinking = True
    if ref in _MODEL_DISPLAY_NAMES:
        base = _MODEL_DISPLAY_NAMES[ref]
    else:
        # strip duplicated provider prefix (anthropic/anthropic/ -> anthropic/)
        parts = ref.split("/")
        if len(parts) >= 2 and parts[0] == parts[1]:
            ref = "/".join(parts[1:])
        base = ref
    return base + (" (no thinking)" if no_thinking else "")


def _discovered_model_response(model_id: str, *, display_name: str) -> ModelResponse:
    return ModelResponse(
        id=model_id,
        display_name=_human_display_name(display_name),
...
async def create_message(
    ...
    # Capture the genuine client headers so the native Anthropic provider can
    # replay them upstream (subscription passthrough that looks like Claude Code).
